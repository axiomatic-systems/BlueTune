/*****************************************************************
|
|   BlueTune - OpenCORE AAC Decoder Module
|
|   (c) 2010 Gilles Boccon-Gibod
...
typedef struct {
    ...
    BLT_UInt32                 mp4es_type_id;
    void*                      opencore_decoder;
    tPVMP4AudioDecoderExternal opencore_config;
                          ...
                          BLT_MediaPacket*    packet)
{
    ...
    if (self->opencore_decoder == NULL) {
        ...
        /* create the OpenCORE decoder */
        self->opencore_decoder = ATX_AllocateZeroMemory(PVMP4AudioDecoderGetMemRequirements());
        if (self->opencore_decoder == NULL) return BLT_ERROR_OUT_OF_MEMORY;
        
        /* configure the decoder */
        ATX_SetMemory(&self->opencore_config, 0, sizeof(self->opencore_config));
        self->opencore_config.outputFormat             = OUTPUTFORMAT_16PCM_INTERLEAVED;
        self->opencore_config.aacPlusEnabled           = 1;
        self->opencore_config.aacPlusUpsamplingFactor  = 2;
        self->opencore_config.desiredChannels          = 2;
        self->opencore_config.pInputBuffer             = (UChar*)mp4_media_type->decoder_info;
        self->opencore_config.inputBufferCurrentLength = mp4_media_type->decoder_info_length;
        
        /* initialize the decoder */
        oc_result = PVMP4AudioDecoderInitLibrary(&self->opencore_config, self->opencore_decoder);
        if (oc_result != MP4AUDEC_SUCCESS) {
            ATX_LOG_WARNING_1("PVMP4AudioDecoderInitLibrary failed (%d)", oc_result);
            return BLT_FAILURE;
        }        

        oc_result = PVMP4AudioDecoderConfig(&self->opencore_config, self->opencore_decoder);
        if (oc_result != MP4AUDEC_SUCCESS) {
            ATX_LOG_WARNING_1("PVMP4AudioDecoderConfig failed (%d)", oc_result);
            return BLT_ERROR_INVALID_MEDIA_FORMAT;
        }
        
        self->sample_buffer_size = BLT_AAC_FRAME_SIZE*2*self->opencore_config.desiredChannels*2; /* the last *2 is for SBR support */
    }

    {
        ...
        out_buffer = (Int16*)BLT_MediaPacket_GetPayloadBuffer(out_packet);
        self->opencore_config.pInputBuffer = in_buffer;
        self->opencore_config.inputBufferCurrentLength = in_size;
        self->opencore_config.pOutputBuffer = out_buffer;
        self->opencore_config.pOutputBuffer_plus = out_buffer+BLT_AAC_FRAME_SIZE*self->opencore_config.desiredChannels;
        self->opencore_config.inputBufferUsedLength = 0;
        self->opencore_config.remainderBits = 0;
        
        oc_result = PVMP4AudioDecodeFrame(&self->opencore_config, self->opencore_decoder);
        if (result != MP4AUDEC_SUCCESS) {
            ATX_LOG_FINER_1("PVMP4AudioDecodeFrame failed (%d)", oc_result);
        ...
        if (self->output.media_type.channel_count == 0) {
            ...
            self->output.media_type.channel_count   = self->opencore_config.desiredChannels;
            self->output.media_type.sample_rate     = self->opencore_config.samplingRate;
            ...
            if (ATX_BASE(self, BLT_BaseMediaNode).context) {
                ...
                stream_info.sample_rate   = self->opencore_config.samplingRate;
                stream_info.channel_count = self->opencore_config.encodedChannels;
                stream_info.mask = BLT_STREAM_INFO_MASK_DATA_TYPE    |
                                   BLT_STREAM_INFO_MASK_SAMPLE_RATE  |
                                   BLT_STREAM_INFO_MASK_CHANNEL_COUNT;
                if (self->opencore_config.audioObjectType         == MP4AUDIO_SBR ||
                    self->opencore_config.extendedAudioObjectType == MP4AUDIO_SBR) {
                    stream_info.data_type = "MPEG-4 HE-AAC (SBR)";
                } else if (self->opencore_config.audioObjectType         == MP4AUDIO_PARAMETRIC ||
                           self->opencore_config.extendedAudioObjectType == MP4AUDIO_PARAMETRIC) {
                    stream_info.data_type = "MPEG-4 HE-AAC (PS)";
                }
            ...
            if (self->output.media_type.sample_rate   != (unsigned int)self->opencore_config.samplingRate || 
                self->output.media_type.channel_count != self->opencore_config.desiredChannels) {
                BLT_MediaPacket_Release(out_packet);
                return BLT_ERROR_INVALID_MEDIA_FORMAT;
            }
        ...
        BLT_MediaPacket_SetPayloadSize(out_packet, self->opencore_config.frameLength *
                                                   self->opencore_config.desiredChannels *
                                                   self->opencore_config.aacPlusUpsamplingFactor *
                                                   2);
...
{ 
    ...
    /* destroy the decoder */
    if (self->opencore_decoder) ATX_FreeMemory(self->opencore_decoder);
                ...
                BLT_SeekPoint* point)
{
    ...
    /* reset the decoder */
    if (self->opencore_decoder) PVMP4AudioDecoderResetBuffer(self->opencore_decoder);
...
BLT_MODULE_IMPLEMENT_STANDARD_GET_MODULE(AacDecoderModule,
                                         "AAC Audio Decoder (OpenCORE)",
                                         "com.axiosys.decoder.opencore.aac",
                                         "1.0.0",
                                         BLT_MODULE_AXIOMATIC_COPYRIGHT)
BLT_Result 
BLT_OpenCoreAacDecoderModule_GetModuleObject(BLT_Module** module)
{
