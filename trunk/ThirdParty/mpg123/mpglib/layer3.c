...
/* $Id: layer3.c,v 1.6 2004/07/03 08:01:42 bok Exp $ */
...
static int III_get_side_info_1(PMPSTR mp, struct III_sideinfo *si,int stereo,
 ...
 */
static int III_get_side_info_2(PMPSTR mp, struct III_sideinfo *si,int stereo,
 int ms_stereo,long sfreq,int single)
{
   int ch;
   int powdiff = (single == 3) ? 4 : 0;
                ...
                int (*synth_1to1_ptr)(PMPSTR,real *,int,unsigned char *, int *) )
{
  ...
  for (gr=0;gr<granules;gr++) 
  {
    ...
    memset(hybridIn, 0, sizeof(hybridIn));    /* GBG */
    memset(hybridOut, 0, sizeof(hybridOut));  /* GBG */
